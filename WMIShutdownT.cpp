...
int main() {
    ...
    if (FAILED(hres)) {
        std::cout << "Falha ao inicializar a segurança." << std::endl;
        CoUninitialize();
        return 1;
    }
    ...
    if (FAILED(hres)) {
        std::cout << "Falha ao criar uma instância IWbemLocator." << std::endl;
        CoUninitialize();
        return 1;
    }
    while (true) {
        // Loop para gerar os nomes das máquinas F00400, F00401, ..., F00499
        for (int i = 0; i <= 99; ++i) {
            ...
            std::wcout << L"Processando a máquina: " << machineNameWStr << std::endl;
            ...
            if (FAILED(hres)) {
                std::wcout << L"Falha ao conectar à máquina " << machineNameWStr << std::endl;
                continue;
            }
            ...
            if (FAILED(hres)) {
                std::wcout << L"Falha na consulta WMI para a máquina " << machineNameWStr << std::endl;
                pSvc->Release();
                continue;
            }
            ...
            while (pEnumerator) {
                ...
                if (SUCCEEDED(hres)) {
                    ...
                    if (SUCCEEDED(hres)) {
                        ...
                        if (SUCCEEDED(hres)) {
                            std::wcout << L"Comando de desligamento enviado com sucesso para a máquina " << machineNameWStr << std::endl;
                        }
                        else {
                            std::wcout << L"Falha ao enviar o comando de desligamento para a máquina " << machineNameWStr << std::endl;
                        }
                    }
                }
